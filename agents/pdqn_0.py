...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.actor = actor_class(self.observation_space.shape[0], self.num_actions, **actor_kwargs).to(device)
        self.actor_target = actor_class(self.observation_space.shape[0], self.num_actions, **actor_kwargs).to(device)
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            all_action_parameters = self.actor_param.forward(state, action)
    ...
    def _optimize_td_loss(self):
        ...
        with torch.no_grad():
            ...
            pred_next_action_parameters = self.actor_param_target.forward(next_states, pred_Q_a)
        ...
        Q_val = Q
        with torch.no_grad():
            action_params = self.actor_param(states, Q_val)
        ...
        # step 2
        new_states = []
        for i,s in enumerate(states.numpy()):


            s = np.concatenate((s, action_parameters.numpy()[i]), axis=1)
            new_states.append(s)

        new_states = np.array(new_states)
        states = torch.from_numpy(new_states).to(self.device)
